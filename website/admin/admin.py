...
from .static.admin_utilities import is_admin, get_admins, get_editors, get_max, get_banned, email_help_response, email_missing_response
...
def Home():
    ...
    admins = get_admins()
    editors = get_editors()
    return render_template("admin_home.html", user=current_user, active="Dashboard", m=get_max([admins, editors]), admins=admins, editors=editors)
...
def Bans():
    ...
    if not is_admin():
        return redirect(url_for("home.Home"))
    return render_template("admin_ban.html", user=current_user, active="Bans", banned=get_banned())
...
def HandlePardon():
    ...
    if not is_admin():
        return redirect(url_for("home.Home"))
    user = User.query.filter_by(username=request.form.get("username")).first()
    if not user or not user in get_banned():
...
def HandleHelpResponse():
    ...
    if not email_help_response(help, response):
...
def HandleMissing():
    ...
    if missing.email and response and len(response) > 0:
        if not email_missing_response(missing, response):
            flash("Error sending email, please try again")
